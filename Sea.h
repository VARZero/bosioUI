...
class ScreenInfo{ // 스크린 하나에 대한 정보
    private:
        ...
        map <int, int> Components_List; // 컴포넌트 리스트
    public:
        ScreenInfo(string name, float LR, float UD, float Dep, int *ScrID){ // 스크린 정보 생성시
        ...
        void Add_Componets(); // 컴포넌트를 추가하기 위한 함수
        void Delete_Componets(int Components_ID); // 컴포넌트를 삭제하기 위한 함수
        ~ScreenInfo(){ // 스크린 삭제
...
class Components_Info{ // 컴포넌트 하나에 관련된 정보
    ...
    public:
        ...
        ~Components_Info(){ // 컴포넌트가 삭제될때
            parents -> Delete_Componets(Components_ID); // 스크린에서 컴포넌트 정보를 삭제
        }
};